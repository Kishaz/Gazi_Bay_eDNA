...
alpha <- cbind(shannon = H, richness = richness, pielou = evenness, st_data)
list(alpha)
...
plot_grid(plot.shan + theme(legend.position = "none"), plot.rich + theme(legend.position = "none"), plot.even + theme(legend.position = "right"),ncol = 3)

#####BETA DIVERSITY
##Calculate pairwise dissimilarity
## Bray-Curtis Dissimilarity
data.mdf <- as.matrix.data.frame(data)
rownames(data.mdf) <- st_data$Site

data.bray <- vegdist(data.mdf, site = "bray")
data.bray

##Jaccard Dissimilarity
data.jac <- vegdist(data.mdf, site = "jaccard", binary = T)
data.jac
##SIMPER
simper(data, st_data$Site, permutations = 999)
...
# create a PCoA plot
pcoa.data.bray.plot <- ggplot(pcoa.data.bray.plotting, aes(x = axis_1, y = axis_2, colour = Site)) +
  geom_point(size = 3) +
  scale_colour_viridis_d(option = "magma", begin = 0.2, end = 0.8) +
  theme_bw() + 
  xlab("PCoA 1 (30.8%)") +
...
pcoa.data.jac.plotting$Site <- rownames(pcoa.data.jac.plotting)

pcoa.data.jac$eig[1]/(sum(pcoa.data.jac$eig))

pcoa.data.jac$eig[2]/(sum(pcoa.data.jac$eig))

pcoa.data.jac.plot <- ggplot(pcoa.data.jac.plotting, aes(x = axis_1, y = axis_2, colour = Site)) +
  geom_point(size = 3) +
  scale_colour_viridis_d(option = "magma", begin = 0.2, end = 0.8) +
  theme_bw() + 
  xlab("PCoA 1 (31.6%)") +
