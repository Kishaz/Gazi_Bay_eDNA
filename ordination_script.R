...
data.envfit <- envfit(data.mds, st_data, permutations = 999) # this fits environmental vectors
data.spp.fit <- envfit(data.mds, data, permutations = 999) # this fits species vectors

## score the mds.
site.scrs <- as.data.frame(scores(data.mds, display = "sites")) #save NMDS results into dataframe
...
##Transform species data to vectors
...
##Plot significant species and habitat patterns
nmds.plot.data+
...
### Permutation multivariate analysis of variance (adonis/Permutational MANOVA/ Nonparametric MANOVA)
data.adonis <- adonis(data.dist ~ Habitat+`Salinity (ppt)`+`Temperature(0C)`, data=st_data)
data.adonis
