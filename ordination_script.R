...
##Performed a non-metric multidimensional scaling with; -two dimensions (k=2) -using principle coordinates analysis to create the initial configuration -used a maximum of 50 random starts & 100 trymax -use (modified) Kruskal's stress to determine the match between ordination distances and ecological distances -use Procrustes rotation to determine whether the configurations have converged -center the axes scores and rotated the points such that the greatest plane of variation is orientated with the primary axis.
data.nmds <- metaMDS(data, try=50, trymax = 100)
...
NMDS = data.frame(NMDS1 = data.nmds$points[,1], NMDS2 = data.nmds$points[,2], Habitat=st_data$Habitat)
...
nmds.plot.data+
  ...
  stat_ellipse(data= NMDS, aes(NMDS1, NMDS2, linetype = Habitat, colour = Habitat)) 
